...
public class BatchLongDarshanGraph {
	...
	private TreeMap<Long, Long> UserIndex;
	private TreeMap<Long, Long> JobIndex;
	private TreeMap<Long, Long> FileIndex;
	...
	public BatchLongDarshanGraph(String darshan_log, String db_file){
		this.path = darshan_log;
		bi = BatchInserters.inserter(db_file);
		
		this.UserIndex = new TreeMap<Long, Long>();
		this.JobIndex = new TreeMap<Long, Long>();
		this.FileIndex = new TreeMap<Long, Long>();
	...
	public void loadLog() throws NumberFormatException, IOException{
		...
		String line;
		long l = 0;
		//as it is too big, we only process first 30k lines
		while ((line = br.readLine()) != null && l < 10000) {
			long time_ts = System.currentTimeMillis();
			System.out.print("Process line " + (l));
			...
			System.out.print(" " + wrops + "");
			Long user;
			Long big_user_id = parseUint64(user_id);
			...
			Long job;
			Long big_job_id = parseUint64(job_id);
			...
			Long exe;
			Long big_exe_id = parseUint64(obj_id);
			...
			for (int i = 0; i < wrops; i++){
				try{
					...
					Long file;
					Long big_file_id = parseUint64(file_id);
			...
			System.out.println(" " + (System.currentTimeMillis() - time_ts));
			l++;
		}
		this.user.flush();
		this.job.flush();
	...
	public static void main(String[] args) throws NumberFormatException, IOException{
		BatchLongDarshanGraph dg = new BatchLongDarshanGraph("/Users/daidong/Documents/gitrepos/graph/darshan-graph/log-2013-1-9.txt", 
